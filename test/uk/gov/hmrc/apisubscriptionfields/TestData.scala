...
trait FieldsDefinitionTestData extends TestData {
  ...
  final val FakeFieldsDefinitions = NEL.fromListUnsafe(List(FakeFieldDefinitionUrl, FakeFieldDefinitionString, FakeFieldDefinitionSecureToken))
  ...
  final val FakeFieldsDefinitionsWithRegex = NEL.fromListUnsafe(List(FakeFieldDefinitionAlphnumericField, FakeFieldDefinitionPassword))
  ...
  def createFieldsDefinition(apiContext: String = fakeRawContext, apiVersion: String = fakeRawVersion, fieldDefinitions: NEL[FieldDefinition] = FakeFieldsDefinitions) =
