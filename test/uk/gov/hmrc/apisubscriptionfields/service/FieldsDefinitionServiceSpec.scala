...
import uk.gov.hmrc.apisubscriptionfields.model.{ApiContext, ApiVersion, ApiFieldDefinitions, BulkFieldsDefinitionsResponse, FieldsDefinitionResponse}
...
class FieldsDefinitionServiceSpec extends UnitSpec with FieldDefinitionTestData with MockFactory {
  ...
  "upsert" should {
    "return false when updating an existing fields definition" in {
      (mockApiFieldDefinitionsRepository save _) expects FakeFieldsDefinition returns ((FakeFieldsDefinition, false))
    ...
    "return true when creating a new fields definition" in {
      (mockApiFieldDefinitionsRepository save _) expects FakeFieldsDefinition returns ((FakeFieldsDefinition, true))
